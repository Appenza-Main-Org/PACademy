...
- **Live demo:** https://admin-prod.appenzademo.com (production) · https://admin-staging.appenzademo.com (staging). Both domains live on the Vercel project `pa-cademy-external`; the bare `appenzademo.com` apex and the old `pa-cademy.vercel.app` URL are dead.
...
| (untagged, 2026-06-06) | Barcode on-prem app — full `features/barcode` slice: on-prem login surface, `barcode:*` role-scoped access, generate/print/reprint, search, group print, admin format/content/layout config — see §11 |
| (untagged, 2026-06-07 → 2026-06-09) | ZKBioTime (BioTime 8.x) biometric integration — `Biometric:Mode=zkbiotime` gateway: enroll→personnel, verify→transaction-poll, NID as device emp_code, live heartbeat, area transfer, incremental punch polling — see §11 |
| (untagged, 2026-06-05 → 2026-06-10) | Data-Exchange hardening: curated full-DB snapshot export, strict cycle scoping, committee-name resolution, reconciliation UX, export gap closures — see §11 |
| (untagged, 2026-06-10) | operational-records normalization wave 2 (13 buckets → typed Shape-A tables) + committee↔category end-to-end validation + snapshot-export restructure (unique row ids, internal sheets dropped, exam-plan-resolved exam columns) + perf pass (memoized export reads, grades by-NID seek, RCSI ON for both DBs) + unique export file names — see §11 |
| (untagged, 2026-06-11 → 2026-06-15) | auth hardening (no hardcoded creds, show-password, remember-me) + weighted Egyptian name engine (byte-identical FE/BE) + per-category education-field config + data-exchange exam-plan-per-category export & ExamReservations import seam (internal-repo port prep) + permanent applicant barcode at payment + acquaintance-doc read-only/autosave + reports overview from live data — see §11 |
...
- **Biometric device gateway** — `Biometric:Mode = simulated | real | zkbiotime` ([backend/admin/PACademy.Admin.Api/Modules/Biometric/](backend/admin/PACademy.Admin.Api/Modules/Biometric/)), mirrors the MOI seam. Simulated gateway returns deterministic match scores; real gateway is a 503 stub until a device is configured; `zkbiotime` is a live ZKTeco BioTime 8.x adapter (2026-06-07 → 06-09 wave, see §11) — enroll maps to a personnel record, verify polls transactions, NID is the device `emp_code`.
...
✅ **Done (Barcode on-prem app, 2026-06-06)**

Full `features/barcode` vertical slice built as an **on-prem** application (separate RBAC plane — `barcode:*` permissions are not in the cloud matrix):

- **On-prem login surface** with role-scoped access; barcode tile on the staff login picker.
- **Generate + print** with copy count; **reprint vs replace** are distinct affordances (replace voids the old barcode).
- **Search** by barcode value, national id, or name; **group print** by category, exam type, committee, or qualification with eligibility gates per role (committee / exam / medical scoping).
- **Admin configuration** for barcode format, content, and layout. Codes stay ASCII (Code128 constraint).

✅ **Done (ZKBioTime biometric integration, 2026-06-07 → 2026-06-09)**

The Biometric seam gained a third mode: `Biometric:Mode = zkbiotime` — a real adapter for ZKTeco **BioTime 8.x** alongside `simulated`/`real` ([backend/admin/PACademy.Admin.Api/Modules/Biometric/](backend/admin/PACademy.Admin.Api/Modules/Biometric/)). BioTime has no synchronous verify API, so the adapter maps **enroll → personnel record** and **verify → transaction polling**:

- **National id is the device `emp_code`** — verify-by-NID works at the device without a separate mapping table; device picker with duplicate guard; an applicant can be **registered on additional devices** after creation.
- **Live ZK server heartbeat** + dynamic verify modality in the admin UI; activation from the admin-saved connection (no redeploy); verify-live is independent of the ZK server clock offset.
- **Area transfer + incremental punch polling**; real history export; history page polish.

✅ **Done (Data-Exchange hardening, 2026-06-05 → 2026-06-10)**

A sustained wave converging on trustworthy exports from `/admin/data-exchange`:

- **Curated full-database snapshot export** — fixed human-readable columns per sheet (the `CuratedSheets` registry in [DataExchangeService.cs](backend/admin/PACademy.Admin.Api/Modules/DataExchangeAdmin/DataExchangeService.cs), mirrored by frontend `EXPORT_DOMAINS`), distinct from the legacy round-trip export used for import templates.
- **Strict cycle scoping** — selected cycle honored end-to-end (applicants, cycle-owned sheets, AdmissionConditions, payments ledger); booked applicants with blank/stale cycle ids are kept, positively-different cycles dropped.
- **Committee-name resolution** — exports and the roster resolve real committee names (booked slot id → category+date schedule lookup → stored-label fallback that rejects category contradictions) instead of stale labels.
- **Export gap closures** — Payments sheet populated (ledger + portal payments + draft snapshot merge), reservations/acquaintance-docs/education/condition-category gaps closed, test names shown in applicant review.
- **Reconciliation UX** — bulk-approve (per-row selection dropped), scrollable list.

✅ **Done (normalization wave 2 + committee-category validation + export restructure + perf, 2026-06-10)**

- **Operational-records normalization wave 2** — 13 more JSON buckets decomposed into typed Shape-A tables (migrations `NormalizeNotificationsExamPlansCommitteeResults`, `NormalizeWorkflowsCommittees`): exam plans, notifications, committee results, workflows, committees, exam devices/results/attempts, payments ledger, and more. Legacy document store is guarded against stale normalized-module access; applicant committee names resolve from the normalized committee tables.
- **Committee ↔ category validation end-to-end** — a committee instance can only pair a committee with its own category (`COMMITTEE_CATEGORY_MISMATCH` conflict, documented in [docs/DB_CONSTRAINTS.md](docs/DB_CONSTRAINTS.md)). Four-layer seam: booking validates server-side; category change clears the booking; display ignores provable mismatches; instance writes 409 on cross-category. Gender is carried in the committee name («طالبات»).
- **Snapshot-export restructure** — every sheet carries a stable per-row unique key (`relative_id`, `result_id`, `condition_id`, `lookup_row_id`, …); internal/system sheets (Committees, ApplicantCategories, Faculties, Notifications, WorkflowRecords, AuditEntries) dropped from the export (kept importable for old workbooks). `ExamSchedules` = `slot_id, exam_id, exam_name, category, date, committee_name, capacity, reserved` — `exam_id`/`exam_name` resolve through the cycle's ordered exam plan via `CuratedExamResolver` (mirrors `/admin/committees-exam-config` labeling) so they are never empty, for schedule rows and every booked applicant's `ExamReservations` row alike. `time`/`location` dropped (appointments are day-granular — the roster UI also shows date only). **Export file names are unique**: `data-exchange-{cycle}-{yyyyMMdd-HHmmss}.xlsx` from the backend export watermark.
- **Performance pass** (the page was taking 16.6s to export 530 rows and 25s+ to list 4 applicants):
  - Snapshot export memoizes the applicant payloads + exam resolver per export (`CuratedContext` lazies) — previously 6 sheets each re-ran the heavy normalized applicants UNION query. Warm export ≈ 0.7s.
  - `ListGradesByNationalIdAsync` — per-applicant grade lookups (eligibility, grade-existence guard) seek `IX_applicant_grades_nid` instead of materializing the whole `applicant_grades` table (~50k rows / 40MB on staging).
  - `OperationalRecordsService.ListAsync` gained an `enrichApplicantCommitteeNames: false` opt-out for callers that resolve committee names themselves.
  - **`READ_COMMITTED_SNAPSHOT` is ON for both `DB_PAcademy_Staging` and `DB_PAcademy_Prod`** (DB-level setting, applied 2026-06-10, not in migrations) — readers no longer block behind in-flight import/write transactions, which caused intermittent 20-30s page loads.
- **Reports** — by-department report groups directly on the canonical category key (was emitting duplicate `officers_general` keys from raw-string slugging; React duplicate-key warnings on `/admin/reports`).
- **Applicant-flow fixes** — family step (guardian can be an existing member, duplicate family NIDs blocked, validation/progress/saved-data hydration synced, family data in print card), exam-schedule honors min-lead-time config (locked by test), published-declaration loading fixed, acquaintance-doc open/close derived from general-settings + cycle override, payment fee synced with active cycle, law-specialization display, eligibility gender handling, dynamic admission terms.
- **Admin fixes** — applicants management gained an admission-cycle filter; admin-created applicants default to the active cycle; education labels/degrees localized with complete projection; NID checksum validation removed (length + structure stay); applicant-grades summary counts; remaining nullable GETs coalesced against the 204-as-undefined TanStack trap.
- **Live demo domains corrected** — production `admin-prod.appenzademo.com`, staging `admin-staging.appenzademo.com` (Vercel project `pa-cademy-external`); `test:routes:prod` retargeted (74/74 routes pass).

✅ **Done (auth hardening + identity name engine + per-category education fields + data-exchange exam-plan/reservation seam + applicant-flow fixes, 2026-06-11 → 2026-06-15)**

- **Auth hardening** ([frontend/src/features/auth/](frontend/src/features/auth/)) — dropped the hardcoded demo login credentials from the staff `LoginForm`; added a show-password toggle (optically centered on the text line) and a **remember-me** option on staff login. Login is now credential-driven only.
- **Weighted Egyptian name engine** — derived test/applicant identities now use a deterministic weighted Egyptian name generator on both sides: `ArabicNameGenerator.cs` (backend) and `arabic-names.ts` (frontend) must stay **byte-identical** (FNV-1a-seeded, 45/35/20 frequency tiers). Retired placeholder names are regenerated for existing applicants; rotated super-admin credentials are now preserved across reseeds. See the [Arabic name engine memory](#) — edit both files together.
- **Per-category education-field config** — admins configure which educational score fields apply per category in admission-setup ([category education-fields editor](backend/admin/PACademy.Admin.Api/Modules/Admissions/CategoryEducationFieldsService.cs), migration `AddCategoryEducationFields`; new `educationFields.service.ts`/`.queries.ts` on the applicant side). The applicant Stage-3/4/5 data page (`Stage345ApplicantDataPage.tsx`) drives its score fields + zod schema from that config; imported-grades path unblocks profile submit; score values are trimmed before config validation; imported branch vocabulary is normalized into `thanawiType`.
- **Permanent barcode after payment** — the applicant gets a **permanent barcode generated at payment time** (sequenced server-side via the `AddBarcodeSequences` migration + `PortalService`), surfaced on the exam card / Stage-9 print card. Replaces the old derived `{nationalId}-{paymentReference}` value for the post-payment card.
- **Acquaintance-doc (Stage 11) hardening** — reuses the applicant's already-submitted personal data and locks it **read-only**; version-guarded autosave (last-chance autosave on close); document stays editable until the closing test. See the [204/null TanStack trap memory](#) for the «نشر»-publish nuance nearby.
- **Data-exchange — exam-plan + reservation seam** (prep for the internal-repo port, see the [internal data-exchange port memory](#)):
  - **Exams sheet exports the configured exam plan per category** (falls back to the default plan when a category has none) — `exam_id`/`exam_name` resolve through the cycle's ordered plan.
  - **ExamReservations import** into applicant scheduling records — hardened reservation matching, exam-results joined in reconciliation, applicant import exam-schedule synced, skipped exam-sync replayed. **Later-stage exam reservations import without a committee instance** (only the first-planned exam requires an instance; later stages key on `appointment_date` — see the [reservation import gate memory](#)).
  - **Curated applicant imports accepted**; pre-booking applicants exported for the internal seed; internal return-workbook imports guarded; active-cycle export scope fixed.
  - Import preview paginated; `ExportInfo` metadata sheet dropped from the export; export copy says «جدول» (not «ورقة») and uses admissions-scope label wording.
- **Reports overview from live data** — `/admin/reports` overview datasets derive from live operational rows (was mock-derived); grouped filter toolbar + hero KPI tiles + heatmap stretch. See the [reports overview real data memory](#) (medical/board sections stay empty — on-prem).
- **Admin applicant detail** — relatives records displayed (backend family projection now includes full relative fields); current-status card refocused as a **payment summary**; published-cycle row highlighted on the cycles list.
- **Applicants list** — pinned row-actions column + click-row-to-open; visible row-actions trigger; committee column surfaced without horizontal scroll; single-fetch list retrieval (perf).
- **Lookups** — allow **add on code-less lookups** (e.g. police-stations, which have no English code).
- **Applicant print card** — qualification + job lookup labels resolved; family info cards wrapped and aligned to equal heights.
...
| Biometric device gateway | [backend/admin/PACademy.Admin.Api/Modules/Biometric/](backend/admin/PACademy.Admin.Api/Modules/Biometric/) — `IBiometricDeviceGateway` behind `Biometric:Mode = simulated \| real \| zkbiotime`. Real gateway returns 503 until configured; `zkbiotime` drives a live BioTime 8.x server (enroll→personnel, verify→transaction-poll, NID as `emp_code`). Mirrors the MOI seam. |
...
| Data Exchange center | [frontend/src/features/data-exchange/](frontend/src/features/data-exchange/) (`/admin/data-exchange`) — centralized Excel import/export hub. Backend reads via `OperationalRecordsService`; backend module under [backend/admin/PACademy.Admin.Api/Modules/DataExchangeAdmin/](backend/admin/PACademy.Admin.Api/Modules/DataExchangeAdmin/). Applicants export gated on first-exam booking. The download is the **curated snapshot** (`CuratedSheets` registry, fixed columns, unique per-row ids, cycle-scoped); file name `data-exchange-{cycle}-{yyyyMMdd-HHmmss}.xlsx`. New sheets that read applicants must use `ctx.ApplicantsAsync()` (memoized per export) — never `records.ListAsync("applicants")` inside a loader. |
| Operational records (admin) | [backend/admin/PACademy.Admin.Api/Modules/OperationalRecords/](backend/admin/PACademy.Admin.Api/Modules/OperationalRecords/) — normalized SQL columns; wave 1 (2026-05-31) covered applicants/grades, wave 2 (2026-06-10) added 13 more buckets (exam plans, notifications, committees, workflows, …). `AdminRecords` retained for document-shaped buckets only. Per-applicant grade reads go through `ListGradesByNationalIdAsync` (index seek), never a full `ListAsync("grades")`. |
...
| Live demo URL | https://admin-prod.appenzademo.com (prod) · https://admin-staging.appenzademo.com (staging) — Vercel project `pa-cademy-external`. The bare `appenzademo.com` apex and `pa-cademy.vercel.app` are dead — don't link them. |
...
| Per-category education fields | [CategoryEducationFieldsService.cs](backend/admin/PACademy.Admin.Api/Modules/Admissions/CategoryEducationFieldsService.cs) (migration `AddCategoryEducationFields`) + applicant `educationFields.service.ts`/`.queries.ts`. Drives the Stage-3/4/5 score fields + zod schema in [Stage345ApplicantDataPage.tsx](frontend/src/features/applicant-portal/pages/Stage345ApplicantDataPage.tsx). |
| Applicant permanent barcode | Sequenced server-side in [PortalService.cs](backend/admin/PACademy.Admin.Api/Modules/ApplicantPortal/PortalService.cs) (migration `AddBarcodeSequences`), generated at payment, shown on [Stage9PrintCardPage.tsx](frontend/src/features/applicant-portal/pages/Stage9PrintCardPage.tsx). |
| Egyptian name engine | `ArabicNameGenerator.cs` (backend) ↔ `arabic-names.ts` (frontend) — must stay byte-identical (FNV-1a-seeded, 45/35/20 tiers). Edit both together. |

---

## 14. Context-document index

### Audience entry points (start here by role)

This CLAUDE.md is the **engineering** operating context. Non-engineering readers should start at their own guide, which routes to the right source docs:

| You are… | Start here |
|---|---|
| **Testing / QA** (writing bugs, test cases) | [docs/TESTING_GUIDE.md](docs/TESTING_GUIDE.md) — env + URLs + test credentials + expected-behavior sources + the [DB_CONSTRAINTS.md](docs/DB_CONSTRAINTS.md) negative-case catalog |
| **Product** (writing user stories, scoping) | [docs/PRODUCT_GUIDE.md](docs/PRODUCT_GUIDE.md) — BRD → current-state → open-gaps map, personas, surface/route inventory |
| **Engineering / Claude Code** | this file (CLAUDE.md), then the affected feature's `index.ts` |
...
| [docs/TESTING_GUIDE.md](docs/TESTING_GUIDE.md) | QA entry point — environments, test accounts/NIDs, expected-behavior sources, negative-test catalog, bug-report template |
| [docs/PRODUCT_GUIDE.md](docs/PRODUCT_GUIDE.md) | Product entry point — spec → current-state → gaps, personas, story-writing checklist |
| [docs/police_academy_detailed_brd_scope_md.md](docs/police_academy_detailed_brd_scope_md.md) | **The BRD** — official RFP requirements, sectioned by app + applicant workflow stage. Story source of truth. |
| [docs/BRD_TESTSPRITE_SCOPE.md](docs/BRD_TESTSPRITE_SCOPE.md) | BRD→test-case map + admin/applicant test data + Playwright checkpoints |
| [docs/APPLICANT_ELIGIBILITY_TEST_MATRIX.md](docs/APPLICANT_ELIGIBILITY_TEST_MATRIX.md) | 40+ NID samples with expected eligibility per category (staging, active cycle) |
| [docs/DB_SCHEMA.md](docs/DB_SCHEMA.md) | Live DB schema map (⚠️ pre-2026-06-10-normalization — regenerate before relying on exact table shapes) |
