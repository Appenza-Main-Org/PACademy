...
public sealed class AddAdjustmentUseCase(IGradesDbContext db, IIdentityApi identity)
{
    public async Task<GradeRowDto> ExecuteAsync(
        int seat, AddAdjustmentRequest req, CancellationToken ct = default)
    {
        ...
        row.AddAdjustment(
            GradeMapper.ParseReason(req.Reason),
            req.Note,
            req.Amount,
            req.IsActive,
            actor.Id);
