...
public sealed class NotificationDepartment : SimpleLookupBase
{
    private NotificationDepartment() { }

    public static NotificationDepartment Create(
        string key, string labelAr, string? labelEn = null,
        int sortOrder = 0, bool isSystem = false, bool demoOrigin = false)
    {
        return new NotificationDepartment
        {
            Id = Guid.NewGuid(),
            Key = key,
            LabelAr = labelAr,
