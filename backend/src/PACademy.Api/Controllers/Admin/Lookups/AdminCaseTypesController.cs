...
public sealed class AdminCaseTypesController(
    ...
    : ControllerBase
{
    [HttpGet] public async Task<ActionResult<PagedResult<CaseTypeDto>>> List([FromQuery] LookupListFilters f, CancellationToken ct)
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<CaseTypeDto>> Get(Guid id, CancellationToken ct)
        => await get.ExecuteAsync(id, ct) is { } dto ? Ok(dto) : NotFound();
    [HttpPost] public async Task<ActionResult<CaseTypeDto>> Create([FromBody] CreateCaseTypeRequest req, CancellationToken ct)
    { var dto = await create.ExecuteAsync(req, ct); return CreatedAtAction(nameof(Get), new { id = dto.Id }, dto); }
    [HttpPatch("{id:guid}")] public async Task<ActionResult<CaseTypeDto>> Update(Guid id, [FromBody] UpdateCaseTypeRequest req, CancellationToken ct)
