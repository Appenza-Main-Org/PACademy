...
public sealed class AdminRanksController(
    ...
    : ControllerBase
{
    [HttpGet] public async Task<ActionResult<PagedResult<RankDto>>> List([FromQuery] LookupListFilters f, CancellationToken ct)
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<RankDto>> Get(Guid id, CancellationToken ct)
        => await get.ExecuteAsync(id, ct) is { } dto ? Ok(dto) : NotFound();
    [HttpPost] public async Task<ActionResult<RankDto>> Create([FromBody] CreateRankRequest req, CancellationToken ct)
    { var dto = await create.ExecuteAsync(req, ct); return CreatedAtAction(nameof(Get), new { id = dto.Id }, dto); }
    [HttpPatch("{id:guid}")] public async Task<ActionResult<RankDto>> Update(Guid id, [FromBody] UpdateRankRequest req, CancellationToken ct)
