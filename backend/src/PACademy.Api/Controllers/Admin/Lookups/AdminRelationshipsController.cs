...
public sealed class AdminRelationshipsController(
    ...
    : ControllerBase
{
    [HttpGet] public async Task<ActionResult<PagedResult<RelationshipDto>>> List([FromQuery] LookupListFilters f, CancellationToken ct)
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<RelationshipDto>> Get(Guid id, CancellationToken ct)
        => await get.ExecuteAsync(id, ct) is { } dto ? Ok(dto) : NotFound();
    [HttpPost] public async Task<ActionResult<RelationshipDto>> Create([FromBody] CreateRelationshipRequest req, CancellationToken ct)
    { var dto = await create.ExecuteAsync(req, ct); return CreatedAtAction(nameof(Get), new { id = dto.Id }, dto); }
    [HttpPatch("{id:guid}")] public async Task<ActionResult<RelationshipDto>> Update(Guid id, [FromBody] UpdateRelationshipRequest req, CancellationToken ct)
