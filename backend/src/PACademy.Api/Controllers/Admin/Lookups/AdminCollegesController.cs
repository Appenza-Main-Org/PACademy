...
public sealed class AdminCollegesController(
    ...
    : ControllerBase
{
    [HttpGet] public async Task<ActionResult<PagedResult<CollegeDto>>> List([FromQuery] CollegeListFilters f, CancellationToken ct)
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<CollegeDto>> Get(Guid id, CancellationToken ct)
        => await get.ExecuteAsync(id, ct) is { } dto ? Ok(dto) : NotFound();
    [HttpPost] public async Task<ActionResult<CollegeDto>> Create([FromBody] CreateCollegeRequest req, CancellationToken ct)
    { var dto = await create.ExecuteAsync(req, ct); return CreatedAtAction(nameof(Get), new { id = dto.Id }, dto); }
    [HttpPatch("{id:guid}")] public async Task<ActionResult<CollegeDto>> Update(Guid id, [FromBody] UpdateCollegeRequest req, CancellationToken ct)
