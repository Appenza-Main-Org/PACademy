...
public sealed class AdminNationalitiesController(
    ...
    : ControllerBase
{
    [HttpGet] public async Task<ActionResult<PagedResult<NationalityDto>>> List([FromQuery] LookupListFilters f, CancellationToken ct)
    ...
    [HttpGet("{id:guid}")] public async Task<ActionResult<NationalityDto>> Get(Guid id, CancellationToken ct)
        => await get.ExecuteAsync(id, ct) is { } dto ? Ok(dto) : NotFound();
    [HttpPost] public async Task<ActionResult<NationalityDto>> Create([FromBody] CreateNationalityRequest req, CancellationToken ct)
    { var dto = await create.ExecuteAsync(req, ct); return CreatedAtAction(nameof(Get), new { id = dto.Id }, dto); }
    [HttpPatch("{id:guid}")] public async Task<ActionResult<NationalityDto>> Update(Guid id, [FromBody] UpdateNationalityRequest req, CancellationToken ct)
