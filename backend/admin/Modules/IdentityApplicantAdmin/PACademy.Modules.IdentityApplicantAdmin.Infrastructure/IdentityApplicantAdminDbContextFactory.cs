...
/// Mirrors <c>LookupsAdminDbContextFactory</c>: builds the context
/// without booting the full host so EF tooling never trips over
/// startup-time middleware.
/// </summary>
public sealed class IdentityApplicantAdminDbContextFactory
    : IDesignTimeDbContextFactory<IdentityApplicantAdminDbContext>
{
    public IdentityApplicantAdminDbContext CreateDbContext(string[] args)
    {
        ...
        var options = new DbContextOptionsBuilder<IdentityApplicantAdminDbContext>()
        ...
        return new IdentityApplicantAdminDbContext(options);
    }
    ...
    private static string FindAppSettings(string startDir)
    {
        ...
        throw new InvalidOperationException($"Could not locate appsettings.json from {startDir}.");
    }
}
