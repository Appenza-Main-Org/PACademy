...
public sealed class AuthController(IIdentityDbContext db, IAuditSink auditSink) : ControllerBase
{
    ...
    public async Task<ActionResult<object>> Login([FromBody] JsonObject body, CancellationToken ct)
    {
        ...
        return await CompleteLoginAsync(nationalId, mobile, requestedRole, ct);
    }

    [HttpPost("api/auth/login-simple")]
    public async Task<ActionResult<object>> LoginSimple(
        [FromForm] string nationalId,
        [FromForm] string mobile,
        [FromForm] string? role,
        CancellationToken ct)
    {
        return await CompleteLoginAsync(nationalId, mobile, role, ct);
    }

    private async Task<ActionResult<object>> CompleteLoginAsync(
        string? nationalId,
        string? mobile,
