  ...
  switch (key) {
    case 'police-stations':
      return {
        ...
        options: (MOCK.lookups.governorates).map((g) => ({ value: g.code, label: g.name })),
      };
    case 'jobs':
      return {
        label: 'تصفية بالفئة',
        ...
        options: [
          ...
          ...(MOCK.lookups.jobs).filter((j) => j.parentCode === null).map((j) => ({ value: j.code, label: j.name })),
        ],
      };
    case 'announcements':
      return {
        label: 'تصفية بالفئة',
        field: 'categoryCode',
        options: (MOCK.lookups['applicant-categories']).map((c) => ({ value: c.code, label: c.name })),
      };
    ...
    case 'relationships': {
      ...
      return [
        ...
        { key: 'parent', label: 'الأصل',  hideOn: 'sm', render: (r: RelationshipRow) => {
          ...
          const p = (MOCK.lookups.relationships).find((x) => x.code === r.parentCode);
