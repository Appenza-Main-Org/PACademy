# AGENTS.md — Police Academy Admissions Platform (Frontend)

> Persistent context for Codex. Read this first before touching code.
...
The repo is organised as a monorepo with two top-level workspaces. Only `AGENTS.md` lives at the root — every other markdown is under `docs/` or `Tasks/`.

```
PACademy/
├── AGENTS.md           ← this file (operating context for Codex)
├── frontend/           ← React 18 + TS + Vite — this AGENTS.md is primarily about this workspace
...
## 12. Working with this repo — Codex-specific guidance

1. **Always start by reading this AGENTS.md, then the affected feature's `index.ts`** to understand what it exports. For visual changes, also read [POLISH_REPORT.md](docs/POLISH_REPORT.md) §5 (cross-screen coherence wins) and the relevant entry in [docs/polish/SHAPE_BRIEFS.md](docs/polish/SHAPE_BRIEFS.md).
...
| Doc | Purpose |
|---|---|
| [AGENTS.md](AGENTS.md) | This file — operating context for Codex (always read first) |
...
| [Tasks/CLAUDE_CODE_BRIEF.md](Tasks/CLAUDE_CODE_BRIEF.md) | Original Codex project brief | Historical (kickoff context) |
